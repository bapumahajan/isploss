...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if ($_POST['ajax'] === 'raise_complaint') {
        ...
        // Get circuit info
        $circuit_stmt = $pdo->prepare("SELECT * FROM customer_basic_information WHERE circuit_id=?");
    ...
    if ($_POST['ajax'] === 'close_complaint') {
        ...
        $check = $pdo->prepare("SELECT incident_status FROM complaints WHERE docket_no=? AND circuit_id=?");
        $check->execute([$docket_no, $circuit_id]);
        $row = $check->fetch(PDO::FETCH_ASSOC);
        if ($row && $row['incident_status'] === 'open') {
            $close = $pdo->prepare("UPDATE complaints SET incident_status='closed', updated_time=?, remarks=CONCAT(remarks, '\nClosed: ', ?) WHERE docket_no=? AND circuit_id=?");
            $close->execute([date('Y-m-d H:i:s'), $close_remarks, $docket_no, $circuit_id]);
            json_response(['success' => true, 'message' => "Complaint $docket_no closed."]);
        } else {
            json_response(['success' => false, 'message' => "Unable to close: Not found or already closed."]);
        }
    }
    ...
    if ($_POST['ajax'] === 'fetch_data') {
        ...
        $complaints = $pdo->prepare("
            SELECT c.docket_no, c.docket_booking_time, c.incident_status, c.updated_time, f.fault_name, c.remarks
            FROM complaints c
...
<div class="container py-3">
    ...
    <?php if ($circuit): ?>
        ...
        <div class="modal fade" id="editContactModal" tabindex="-1" aria-labelledby="editContactModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form id="editContactForm" class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="text" class="form-control" name="number" id="editContactNumber" required>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="modal fade" id="addEmailModal" tabindex="-1" aria-labelledby="addEmailModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form id="addEmailForm" class="modal-content">
                    <div class="modal-header">
            ...
            <div class="modal-dialog">
                <form id="editEmailForm" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="email" class="form-control" name="email" id="editEmailAddr" required>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-success">Save</button>
                    </div>
