...
$allowed_sorts = ['docket_booking_time DESC', 'docket_booking_time ASC'];
$sort = $_GET['sort'] ?? 'docket_booking_time DESC';
if (!in_array($sort, $allowed_sorts)) $sort = 'docket_booking_time DESC';
...
foreach ($q as $row) $status_counts[$row['incident_status']] = $row['cnt'];
foreach (array_keys($status_labels) as $status) if (!isset($status_counts[$status])) $status_counts[$status] = 0;

// --- FIX: Declare highlight() ONCE, before HTML output ---
function highlight($text, $search) {
    if (!$search) return htmlspecialchars($text);
    return preg_replace('/('.preg_quote($search, '/').')/i', '<span class="highlight">$1</span>', htmlspecialchars($text));
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .tabs-bar { display: flex; align-items: center; gap: 8px; margin-bottom: 2px; }
        .tabs-bar .nav-tabs { flex: 1 1 auto; }
        .raise-complaint-btn { margin-left: auto; }
        @media (max-width: 768px) {
            .raise-complaint-btn { width: 100%; margin-left: 0; margin-top: 8px; }
            .tabs-bar { flex-direction: column; align-items: stretch; }
        }
    </style>
</head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-primary sticky-top shadow-sm py-1" aria-label="Main navigation">
...
<div class="container-fluid px-3 py-3">
    ...
    <!-- Tabs bar with Raise Complaint button on right -->
    <div class="tabs-bar">
        <ul class="nav nav-tabs mb-1" id="dashboardTabs">
            <li class="nav-item">
        ...
        <a href="complaints_portal1.php" class="btn btn-success btn-sm raise-complaint-btn"><i class="bi bi-plus-lg"></i> Raise Complaint</a>
    </div>
    <div class="tab-content">
      ...
      <div class="tab-pane fade show active" id="all">
        ...
        <div class="table-responsive">
            <table class="table table-compact table-bordered table-hover align-middle bg-white shadow-sm rounded" id="complaintsTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Docket</th>
                ...
                <tr class="<?= $colorClass ?>">
                    <td><?= ($offset + $i + 1) ?></td>
                    <td><?= highlight($row['docket_no'], $search) ?></td>
                    <td><?= highlight($row['organization_name'], $search) ?></td>
                    <td><?= highlight($row['circuit_id'], $search) ?></td>
