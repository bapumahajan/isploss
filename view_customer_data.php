    ...
    <div class="dashboard-content">
      <div class="dashboard-main">
        ...
        if (isset($_GET['search'])) {
            ...
            if ($stmt->rowCount() > 0) {
                $main_results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            }

            // 2. Get operator tables dynamically
            $operator_names = [];
            try {
                $op_stmt = $pdo->prepare("SELECT name FROM operators");
                $op_stmt->execute();
                $operator_names = $op_stmt->fetchAll(PDO::FETCH_COLUMN);
            } catch (PDOException $e) {
                error_log("Error fetching operators: " . $e->getMessage());
            }

            // 3. Search for third party info
            $third_parties_grouped = [];
            foreach ($operator_names as $tp_table) {
                try {
                    $tp_stmt = $pdo->prepare("SELECT * FROM `$tp_table` WHERE circuit_id = ? LIMIT 10");
                    $tp_stmt->execute([$search_input]);
                    $data = $tp_stmt->fetchAll(PDO::FETCH_ASSOC);
                    if (!empty($data)) {
                        $third_parties_grouped[$tp_table] = $data;
                    }
                } catch (PDOException $e) {
                    error_log("Error querying $tp_table: " . $e->getMessage());
                }
            }
            ...
            if (!empty($main_results)) {
                foreach ($main_results as $row) {
                    // Fetch additional tab info as before
        ...
        <div class="card">
            ...
            <ul class="nav modern-tabs" role="tablist">
                ...
                <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#thirdparty<?= htmlspecialchars($row['circuit_id']) ?>" type="button"><i class="bi bi-diagram-2"></i> Third Party</button></li>
                <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#portstatus<?= $circuit_id ?>" type="button"><i class="bi bi-plug"></i> Port Status</button></li>
            </ul>
            <div class="tab-content">
                ...
                <div class="tab-pane fade" id="thirdparty<?= htmlspecialchars($row['circuit_id']) ?>">
                    <?php if (!empty($third_parties_grouped)): ?>
                        <?php foreach ($third_parties_grouped as $provider => $providerData): ?>
                            <div class="mb-3">
                                <button class="btn btn-outline-primary w-100 text-start" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_<?= md5($provider.$row['circuit_id']) ?>">
                                    <strong><?= htmlspecialchars($provider) ?></strong>
                                    <span class="badge bg-secondary"><?= count($providerData) ?></span>
                                </button>
                                <div class="collapse mt-2" id="collapse_<?= md5($provider.$row['circuit_id']) ?>">
                                    <?php foreach ($providerData as $tp): ?>
        ...
        <?php
            ...
            // If no main info, but third party info found
            elseif (!empty($third_parties_grouped)) {
        ?>
        <div class="card">
            <div class="card-header">
                <h5>
                    <span class="text-muted ms-2" style="color:#8b8b8b;opacity:.8;">[<?= showval($search_input) ?>]</span>
                </h5>
            </div>
            <ul class="nav modern-tabs" role="tablist">
                <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#thirdparty<?= htmlspecialchars($search_input) ?>" type="button"><i class="bi bi-diagram-2"></i> Third Party</button></li>
            </ul>
            <div class="tab-content">
                <div class="tab-pane fade show active" id="thirdparty<?= htmlspecialchars($search_input) ?>">
                    <?php foreach ($third_parties_grouped as $provider => $providerData): ?>
                        <div class="mb-3">
                            <button class="btn btn-outline-primary w-100 text-start" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_<?= md5($provider.$search_input) ?>">
                                <strong><?= htmlspecialchars($provider) ?></strong>
                                <span class="badge bg-secondary"><?= count($providerData) ?></span>
                            </button>
                            <div class="collapse mt-2" id="collapse_<?= md5($provider.$search_input) ?>">
                                <?php foreach ($providerData as $tp): ?>
                                    <table class="table table-bordered mb-2">
                                        <?php foreach ($tp as $k => $v): ?>
                                        <tr>
